...
rsaWrapper.generate = (direction) => {
    ...
    fs.writeFileSync(path.resolve(path.dirname(__dirname), 'keys', direction + '.private.pem'), key.exportKey('pkcs8-private-pem'));
    fs.writeFileSync(path.resolve(path.dirname(__dirname), 'keys', direction + '.public.pem'), key.exportKey('pkcs8-public-pem'));
